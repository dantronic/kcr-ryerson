...
OsdMaster osdMaster;                                  // 
int lastMillis = 100;                                 // 
int serveLastMillis = -1;                             // 
boolean thisIsAgoodTime = false;                      //
boolean db = false;                                   // q: no movie, 8 stills
String sTest;                                         // w: big movie left half
boolean audioMuted = false;                           // e: big movie middle half
boolean textHidden = true;                            // r: big movie right half
long slidePeriodMs = 5000;                            // 
long crossfadeDurationMs = 2000;                      // a: small movie, 7 stills
float fadeRate = 1 / crossfadeDurationMs;             // s: small movie, big stills left
long osdTimeoutDurationMs = 3000;                     // d: small movie, big stills middle
Movie myMovie;                                        // f: small movie, big stills right
PFont f;                                              //
float fadeIncrement = 0.08;                           // fake crossfades what
                                                      // bench: gobbled up all RAM but reported only 5.00 GB hmm
...
class SuperScreen {                            
  ...
  void serve() {
    ...
    switch(config) {                 // [0][1][2][3]
      ...
      break;
      case 11: switch(tvPoint) {     // [\][/][2][3]
        case 2: tvPoint = 3; break;  // [/][\][6][7]
        case 3: tvPoint = 6; break;
        case 6: tvPoint = 7; break;
        default :tvPoint = 2; break;
      }                
      break;
      case 22: switch(tvPoint) {     // [0][\][/][3]
        case 0: tvPoint = 3; break;  // [4][/][\][7]
        case 3: tvPoint = 4; break;
        case 4: tvPoint = 7; break;
        default :tvPoint = 0; break;
      }                
      break;
      case 33: switch(tvPoint) {     // [0][1][\][/]
        case 0: tvPoint = 1; break;  // [4][5][/][\]
...
class OsdMaster {  // drive the key-responsive message overlays...........................................
  String message;
  ...
  void userSaid(char _k) {
    ...
    switch(_k) {
      ...
      case 'k': 
                ...
                message = "k: Crossfade " + crossfadeDurationMs + " ms";
                osdSay(message);
                break;
      case 'l':
                crossfadeDurationMs -= 200;
                if(crossfadeDurationMs<200) crossfadeDurationMs = 200;
                message = "l: Crossfade " + crossfadeDurationMs + " ms";
                osdSay(message);
                break;
...
void setup() {
  ...
  osdMaster.osdSay("Karachi Central Rail");
}
